...
scaleDrawing = function(drawing0) {
  ...
  drawing = drawing0
  drawing[,1:2] = cbind( 2*(drawing0[,1]-rangeX[1]-.5*(rangeX[2]-rangeX[1]) )/maxDiff,
...
findDistance = function(contour1){
  ...
  #the first point is set to 0
    deltaX = sqrt(   rowSums ( 
      (contour1[2:nrow(contour1),1:2] - contour1[1:(nrow(contour1)-1),1:2] )^2) ) #compute distance between x vars
    result = c(0,deltaX) 
...
distance = function(xy,xypoint) {
  ...
  return(
    (rowSums( 
      (xy - 
         cbind( rep(as.numeric( xypoint[1] ),nrow(xy) ) ,
                rep( as.numeric( xypoint[2] ),nrow(xy) ) )      )^2 )
    )^(0.5) 
  )
}
makePlot = function(drawing_in,doLines=T,trans=1,axes0=T,polygon=F,lwd=3) {
  ...
  if (doLines==T ) {
    ...
    if  (ncol(drawing_in)==2) {lines(drawing_in,lwd=lwd) } else{
